...
function MarketSummaryCard({ market, dark, isMobile }) {
  ...
  // Determine market open/close status
  const now = new Date()
  const ist = new Date(now.toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }))
  ...
  if (isIndia) {
    ...
    // US market: 9:30 PM – 4:00 AM IST (roughly)
  ...
  // Placeholder data when no DB data yet
  ...
  return (
    <div style={{
      ...
      {/* ── Header ── */}
      <div style={{
        padding: isMobile ? '13px 14px 10px' : '15px 18px 11px',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
          ...
          <button
            ...
            style={{
              ...
              border: `1px solid ${borderCol}`, color: textMuted,
              borderRadius: '7px', padding: '4px 9px', fontSize: '11px',
      ...
      <div style={{ display: 'flex', borderBottom: `1px solid ${borderCol}` }}>
        {display.indices.map((idx, i) => (
          <div key={i} style={{
            ...
            <div style={{ fontSize: '9px', fontWeight: '700', letterSpacing: '1.2px', textTransform: 'uppercase', color: textMuted, fontFamily: 'var(--font-ui)', marginBottom: '4px' }}>
              {idx.label}
            </div>
            <div style={{ fontSize: isMobile ? '14px' : '16px', fontWeight: '700', color: textPri, fontFamily: 'var(--font-ui)', letterSpacing: '-0.5px', lineHeight: 1, marginBottom: '3px' }}>
              {loading ? '—' : idx.value}
            </div>
            <div style={{ fontSize: '11px', fontWeight: '600', fontFamily: 'var(--font-ui)', color: idx.up ? '#16A34A' : '#DC2626', display: 'flex', alignItems: 'center', gap: '3px' }}>
        ...
        <>
          ...
          <div style={{
            ...
            {loading ? (
              ...
              <p style={{ margin: 0, fontSize: isMobile ? '12px' : '13px', lineHeight: 1.65, color: textPri, fontStyle: 'italic', fontFamily: 'var(--font-display)' }}>
                {display.headline}
              </p>
            )}
          </div>
          ...
          <div style={{ display: 'flex', borderBottom: `1px solid ${borderCol}` }}>
            {display.tiles.map((tile, i) => (
              <div key={i} style={{
                flex: 1, padding: isMobile ? '10px 10px' : '12px 14px',
                borderRight: i < display.tiles.length - 1 ? `1px solid ${borderCol}` : 'none',
                background: tileBg,
              }}>
                ...
                <div style={{ fontSize: isMobile ? '12px' : '13px', fontWeight: '700', color: textPri, fontFamily: 'var(--font-ui)', marginBottom: '2px' }}>
                  {loading ? '—' : tile.value}
                </div>
          ...
          <div style={{ padding: isMobile ? '12px 14px' : '14px 18px', borderBottom: `1px solid ${borderCol}` }}>
            <div style={{ fontSize: '9px', fontWeight: '700', letterSpacing: '1.5px', textTransform: 'uppercase', color: textMuted, fontFamily: 'var(--font-ui)', marginBottom: '10px' }}>
              Sector Performance
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
              {display.sectors.map((sec, i) => {
                const pct     = sec.pct
                const isPos   = pct >= 0
                const barPct  = Math.round((Math.abs(pct) / maxAbsPct) * 100)
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span style={{ fontSize: isMobile ? '11px' : '12px', color: textSec, fontFamily: 'var(--font-ui)', width: isMobile ? '62px' : '72px', flexShrink: 0 }}>
                      {sec.name}
                    </span>
          ...
          <div style={{ padding: isMobile ? '9px 14px' : '10px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: '10px', color: textMuted, fontFamily: 'var(--font-ui)', display: 'flex', alignItems: 'center', gap: '4px' }}>
              🕔 Last updated: {display.updatedAt}
            </span>
            <span style={{ fontSize: '10px', color: accentColor, fontWeight: '600', fontFamily: 'var(--font-ui)' }}>
              Finance Digest · AI Summary
            </span>
          </div>
        </>
      )}
...
// ── Gamification components (outside Home so React treats them as stable) ─────
...
function PredictionGame({ indices, prediction, predCorrect, afterClose, weekend, dark, isMobile, handlePrediction }) {
  ...
  if (weekend) return (
    <div style={{ borderRadius: '14px', marginBottom: '16px', overflow: 'hidden',
      ...
      <div style={{ padding: '12px 16px', background: 'linear-gradient(90deg, #C9A84C, #E8C97A)',
        display: 'flex', alignItems: 'center', gap: '8px' }}>
        <span style={{ fontSize: '16px' }}>🎯</span>
        <span style={{ fontSize: '11px', fontWeight: '700', color: '#1A1410',
          letterSpacing: '0.1em', fontFamily: 'var(--font-ui)' }}>MARKET PREDICTION</span>
      </div>
      <div style={{ padding: '16px', textAlign: 'center' }}>
        <p style={{ margin: 0, fontSize: '14px', color: dark ? '#6B6055' : '#9A8E7E', fontFamily: 'var(--font-ui)' }}>
          📅 Markets are closed on weekends. Come back Monday!
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ borderRadius: '14px', marginBottom: '16px', overflow: 'hidden',
      ...
      <div style={{ padding: '12px 16px', background: 'linear-gradient(90deg, #C9A84C, #E8C97A)',
        ...
        <span style={{ fontSize: '16px' }}>🎯</span>
        <span style={{ fontSize: '11px', fontWeight: '700', color: '#1A1410',
          letterSpacing: '0.1em', fontFamily: 'var(--font-ui)' }}>DAILY MARKET PREDICTION</span>
        <span style={{ marginLeft: 'auto', fontSize: '10px', color: '#1A1410',
          fontFamily: 'var(--font-ui)', opacity: 0.7 }}>+10 IQ to predict · +30 if correct</span>
      </div>
      <div style={{ padding: '16px' }}>
        {!afterClose && !prediction && (
          <>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => handlePrediction('up')} style={{
              ...
              <button onClick={() => handlePrediction('down')} style={{
                flex: 1, padding: '14px', borderRadius: '12px',
        ...
        {!afterClose && prediction && (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            ...
            <p style={{ margin: '0 0 6px', fontSize: '15px', fontWeight: '700',
              color: prediction === 'up' ? '#4ADE80' : '#F87171', fontFamily: 'var(--font-display)' }}>
              You predicted {prediction === 'up' ? 'UP' : 'DOWN'}!
            </p>
            <p style={{ margin: 0, fontSize: '12px', color: dark ? '#6B6055' : '#9A8E7E', fontFamily: 'var(--font-ui)' }}>
              ⏰ Result revealed at 3:30 PM IST
            </p>
          </div>
        )}
        {afterClose && prediction && (
          <div style={{ textAlign: 'center', padding: '4px 0' }}>
            <div style={{ fontSize: '36px', marginBottom: '8px' }}>
              {predCorrect === null ? '⏳' : predCorrect ? '🎉' : '😅'}
            </div>
            <p style={{ margin: '0 0 6px', fontSize: '16px', fontWeight: '700', fontFamily: 'var(--font-display)',
              color: predCorrect ? '#4ADE80' : '#F87171' }}>
            ...
            <p style={{ margin: 0, fontSize: '11px', color: 'var(--accent)', fontFamily: 'var(--font-ui)', fontWeight: '600' }}>
        ...
        {afterClose && !prediction && (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            <p style={{ margin: 0, fontSize: '14px', color: dark ? '#6B6055' : '#9A8E7E', fontFamily: 'var(--font-ui)' }}>
              Market has closed. Come back tomorrow to predict! 🌙
            </p>
          </div>
        )}
      </div>
...
function NewsQuizSection({ glossaryQuiz, quizLoading, quizAnswers, dark, isMobile, handleQuizAnswer }) {
  if (quizLoading) return (
    <div style={{ borderRadius: '14px', marginBottom: '16px', padding: '20px', textAlign: 'center',
      border: `1px solid ${dark ? '#2C2822' : '#EDE8E0'}`, background: dark ? '#1A1410' : '#fff' }}>
      <p style={{ margin: 0, fontSize: '13px', color: dark ? '#6B6055' : '#9A8E7E', fontFamily: 'var(--font-ui)' }}>
        Loading today's quiz from the news...
      </p>
    </div>
  )

  if (!glossaryQuiz.length) return null

  const totalAnswered = Object.keys(quizAnswers).length
  const totalCorrect  = Object.entries(quizAnswers)
    .filter(([qi, ai]) => parseInt(ai) === glossaryQuiz[parseInt(qi)]?.answer).length

  return (
    <div style={{ borderRadius: '14px', marginBottom: '16px', overflow: 'hidden',
      border: `1px solid ${dark ? '#2C2822' : '#EDE8E0'}`, background: dark ? '#1A1410' : '#fff' }}>

      <div style={{ padding: '12px 16px', background: dark ? '#1e1a14' : '#1A1410',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '15px' }}>📰</span>
          <div>
            <span style={{ fontSize: '11px', fontWeight: '700', color: '#C9A84C',
              letterSpacing: '0.1em', fontFamily: 'var(--font-ui)' }}>FROM TODAY'S NEWS</span>
            <p style={{ margin: '1px 0 0', fontSize: '10px', color: '#6B6055', fontFamily: 'var(--font-ui)' }}>
              Terms from today's articles · +20 IQ per correct answer
            </p>
          </div>
        </div>
        {totalAnswered > 0 && (
      ...
      <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {glossaryQuiz.map((q, qi) => {
          const answered = quizAnswers[qi] !== undefined
          const selected = parseInt(quizAnswers[qi])
          const correct  = answered && selected === q.answer

          return (
            <div key={qi} style={{ borderRadius: '10px', overflow: 'hidden',
              border: `1px solid ${answered
                ? (correct ? 'rgba(22,163,74,0.25)' : 'rgba(239,68,68,0.2)')
                : (dark ? '#2C2822' : '#EDE8E0')}` }}>

              <div style={{ padding: '8px 12px',
                background: dark ? 'rgba(255,255,255,0.03)' : '#F7F4EF',
                display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div style={{ padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                {q.options.map((opt, oi) => {
                  ...
                  return (
                    <button key={oi} onClick={() => handleQuizAnswer(qi, oi, q.answer)} style={{
                      display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between',
                      ...
                      {icon && <span style={{ fontWeight: '700', flexShrink: 0, marginTop: '1px' }}>{icon}</span>}
                    </button>
                  )
                })}
              </div>
        ...
        {totalAnswered === glossaryQuiz.length && (
          <div style={{ padding: '10px 14px', borderRadius: '10px', textAlign: 'center',
            background: totalCorrect >= 4 ? 'rgba(22,163,74,0.08)' : 'rgba(201,168,76,0.08)',
            border: `1px solid ${totalCorrect >= 4 ? 'rgba(22,163,74,0.2)' : 'rgba(201,168,76,0.2)'}` }}>
            <p style={{ margin: 0, fontSize: '13px', fontWeight: '700',
              fontFamily: 'var(--font-display)',
              color: totalCorrect >= 4 ? '#4ADE80' : '#C9A84C' }}>
              {totalCorrect >= 4 ? '🎉' : '💡'} {totalCorrect}/{glossaryQuiz.length} correct · +{totalCorrect * 20} IQ earned today
            </p>
          </div>
        )}
      </div>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const savedQuiz = localStorage.getItem(`fd-quiz-${todayStr}`)
  ...
  useEffect(() => {
    if (activeSection === 'headlines') fetchGlossaryQuiz()
  }, [activeSection])
  ...
  function handlePrediction(dir) {
    ...
    addIQ(10, '+10 IQ! Prediction made ⚡')
  }

  function handleQuizAnswer(qIdx, optIdx, correctIdx) {
    if (quizAnswers[qIdx] !== undefined) return
    const todayStr = new Date().toDateString()
    const updated  = { ...quizAnswers, [qIdx]: optIdx }
    setQuizAnswers(updated)
    localStorage.setItem(`fd-quiz-${todayStr}`, JSON.stringify(updated))
    const correct = optIdx === correctIdx
    addIQ(correct ? 20 : 5, correct ? '+20 IQ! Correct! 🎉' : '+5 IQ! Keep learning 📚')
  ...
  async function fetchGlossaryQuiz() {
    setQuizLoading(true)
    try {
      ...
      data?.forEach(article => {
        ...
        if (Array.isArray(g)) {
          g.forEach(item => {
            if (item.term && item.definition && item.definition.length < 120)
              allTerms.push(item)
          })
        }
      })
      ...
      if (unique.length < 4) { setQuizLoading(false); return }
      ...
      const wrongPool = shuffled.slice(5)

      const questions = selected.map((term, i) => {
        const wrongs  = seededShuffle(wrongPool.filter(t => t.term !== term.term), seed + i)
          .slice(0, 3).map(t => t.definition.slice(0, 90))
        const correct = term.definition.slice(0, 90)
        const opts    = seededShuffle([correct, ...wrongs], seed + i + 99)
        return { term: term.term, options: opts, answer: opts.indexOf(correct) }
      })

      setGlossaryQuiz(questions)
  ...
  const BadgeWall = ({ compact = false }) => (
    <div style={{ marginBottom: compact ? '8px' : '16px' }}>
      {!compact && <p style={{ fontSize: '9px', fontWeight: '700', letterSpacing: '0.1em',
        color: dark ? '#4A4438' : '#C4B9AE', margin: '12px 0 8px 12px',
        fontFamily: 'var(--font-ui)' }}>BADGES</p>}
        ...
        {ALL_BADGES.map(b => {
          ...
          return (
            <div key={b.id} title={`${b.name}: ${b.desc}`} style={{
              ...
              background: earned
                ? (dark ? 'rgba(201,168,76,0.15)' : 'rgba(201,168,76,0.12)')
  ...
  const SkeletonCard = () => (
    <div style={{ background: 'var(--bg-card)', borderRadius: '18px',
  ...
  const headerH = (!isPortfolio && articles.length > 0 && isPro)
    ? (isMobile ? 130 : 108) : (isMobile ? 82 : 72)
        ...
        <aside style={{
          ...
          <div style={{ padding: '20px 12px 40px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ marginBottom: '16px', padding: '0 4px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <div style={{ height: '3px', borderRadius: '2px', marginBottom: '8px',
              ...
              <button onClick={() => setSidebarOpen(false)} style={{
                ...
                color: dark ? '#4A4438' : '#C4B9AE', fontSize: '18px', padding: '4px' }}>✕</button>
            </div>
            ...
            <div style={{ padding: '12px', marginBottom: '12px', borderRadius: '10px',
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                <span style={{ fontSize: '11px', fontWeight: '700', color: 'var(--accent)', fontFamily: 'var(--font-ui)' }}>
                  🧠 Finance IQ
                </span>
                <span style={{ fontSize: '16px', fontWeight: '700', color: iqLevel.color, fontFamily: 'var(--font-ui)' }}>
                  {iqScore}
                </span>
              </div>
              <p style={{ margin: '0 0 8px', fontSize: '10px', color: iqLevel.color, fontFamily: 'var(--font-ui)', fontWeight: '600' }}>
                {iqLevel.title}
              </p>
              <div style={{ height: '4px', borderRadius: '2px', background: dark ? '#2C2822' : '#EDE8E0', overflow: 'hidden' }}>
                <div style={{ height: '100%', borderRadius: '2px',
                  ...
                  width: `${Math.min((iqScore % 500) / 5, 100)}%`, transition: 'width 0.5s ease' }} />
              </div>
            ...
            {DESKTOP_NAV.map((item, i) => {
              if (item.type === 'label') return (
                <p key={i} style={{ fontSize: '9px', fontWeight: '700', letterSpacing: '0.1em',
                  color: dark ? '#4A4438' : '#C4B9AE', margin: '12px 0 4px 12px',
                  fontFamily: 'var(--font-ui)' }}>{item.text}</p>
              )
        ...
        {isMobile ? (
          <div style={{ padding: '10px 16px', display: 'flex', flexDirection: 'column', gap: '7px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h1 style={{ fontSize: '18px', fontWeight: '700', color: dark ? '#F0EBE3' : '#1A1410',
                margin: '0', letterSpacing: '-0.03em', fontFamily: 'var(--font-display)', lineHeight: 1.1 }}>
                Finance <span style={{ color: 'var(--accent)' }}>Digest</span>
              </h1>
          ...
          <div style={{ padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0 }}>
              <button onClick={() => setSidebarOpen(o => !o)} style={{
                ...
                fontSize: '20px', cursor: 'pointer', padding: '4px', lineHeight: 1, flexShrink: 0 }}>☰</button>
              <div>
                <h1 style={{ fontSize: '22px', fontWeight: '700', color: dark ? '#F0EBE3' : '#1A1410',
                  margin: '0', letterSpacing: '-0.03em', fontFamily: 'var(--font-display)', lineHeight: 1.1 }}>
                  Finance <span style={{ color: 'var(--accent)' }}>Digest</span>
                </h1>
                <p style={{ fontSize: '10px', color: dark ? '#4A4438' : '#B8AFA3', margin: '2px 0 0',
                  letterSpacing: '0.07em', textTransform: 'uppercase' }}>
                  {today}
                  {activeSectionLabel && <span style={{ color: 'var(--accent)', marginLeft: '6px' }}>· {activeSectionLabel}</span>}
                </p>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
          ...
          {BOTTOM_TABS.map(tab => {
            ...
            return (
              <button key={tab.id} onClick={() => handleMobileTabClick(tab.id)} style={{
                ...
                gap: '3px', border: 'none', background: 'none', cursor: 'pointer', height: '100%', position: 'relative',
              }}>
      ...
      <main style={{
        ...
        {isPortfolio ? (
          ...
          <div style={{ maxWidth: '820px', margin: '0 auto', padding: isMobile ? '16px 14px 20px' : '32px 24px 72px' }}>
            ...
            {/* ── Market Summary Card — Indian & US markets only ── */}
